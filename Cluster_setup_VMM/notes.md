...
# Step2:
- Use the script **vm_config/run_tasks.sh** to build container image with name ansible-ubuntu22 and imventroy files for Ansible Playbook
...
- Post running the script, use podman to spin up a container to run the ansible playbook
    ...
    ```
    $ podman run --rm -it -v $PWD:/workspace:Z -v $HOME/.ssh:/root/.ssh:Z ansible-ubuntu ansible-playbook -i inventory.ini ansible-playbook.yml
    ```